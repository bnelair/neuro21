...
#include "../../neuro21_cpp/src/gorilla_compression.hpp"

namespace py = pybind11;

py::bytes compress_wrapper(py::array_t<double> data_points) {
    auto buf = data_points.request();
    if (buf.ndim != 1) {
        throw std::runtime_error("Input should be a 1-D array");
    }
    std::vector<double> input_data(static_cast<double*>(buf.ptr), static_cast<double*>(buf.ptr) + buf.size);
...
py::array_t<double> decompress_wrapper(const std::string& compressed_data) {
    ...
    py::array_t<double> result(decompressed_data.size());
    auto buf = result.request();
    std::memcpy(buf.ptr, decompressed_data.data(), decompressed_data.size() * sizeof(double));
    return result;
}
